...
class UserMap extends BaseMap{
    ...
    private function update(User $user)
    {
        ...
        if ($this->db->exec("UPDATE user SET lastname = $lastname, firstname = $firstname, patronymic = $patronymic,
            login = $login, pass = $pass, gender_id = $user->gender_id, birthday = $birthday,
            role_id = $user->role_id, active = $user->active WHERE user_id = " . $user->user_id) == 1) {
            return true;
        }
        return false;
    }
    ...
    public function findProfileById($id = null) {
        if ($id) {
            ...
            CONCAT(user.lastname, ' ', user.firstname, ' ', user.patronymic) AS fio,
            user.login, user.birthday, gender.name AS gender,
            role.name AS role, user.active
            FROM user
