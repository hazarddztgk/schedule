...
class StudentMap extends BaseMap{
    public function findById($id = null) {
        if ($id) {
            $res = $this->db->query("SELECT user_id, gruppa_id, num_zach AS zac FROM student WHERE user_id = $id");
    ...
    public function save(User $user, Student $student)
    {
        if ($user->validate() && $student->validate() && (new UserMap())->save($user))
        {
            ...
            { $student->user_id = $user->user_id; return $this->insert($student); }
            else { return $this->update($student); }
        }
        return false; }

    private function insert(Student $student) {
        $num_zach = $this->db->quote($student->num_zach);
        if ($this->db->exec("INSERT INTO student(user_id, gruppa_id, num_zach) VALUES($student->user_id, $student->gruppa_id, $num_zach)") == 1)
        { return true; }
        return false; }
    ...
    public function findProfileById($id = null) {
        if ($id)
        {
            $res = $this->db->query("SELECT student.user_id, gruppa.name AS gruppa, student.num_zach AS zac FROM student " . "INNER JOIN gruppa ON student.gruppa_id=gruppa.gruppa_id WHERE student.user_id = $id");
            return $res->fetch(PDO::FETCH_OBJ); } return false;
    }
